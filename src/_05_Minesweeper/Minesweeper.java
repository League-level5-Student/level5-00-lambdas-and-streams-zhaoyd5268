...
public class Minesweeper extends PApplet {
	 ...
	 */
	boolean checkWin() {
		if (cells.stream().noneMatch((Cell) -> Cell.revealed==false && Cell.mine == false) == true) {
			return true;
		} else {
			return false;
		}
	 ...
	 *              1 1 - X //
	 *              - - - - // 
	 * When the cell with the X is clicked all cells with '-' should be revealed 
	 */
	void revealCell(Cell cell) {
		...
		if (cell.mine == false) {
			...
			if (cell.minesAround == 0) {
				...
				c.stream().filter((Cell)-> Cell.mine == false).forEach((Cell) -> Cell.revealed = true);
				cell = c.get(ran.nextInt(c.size()));
				revealCell(cell);
			}
		}
	}
